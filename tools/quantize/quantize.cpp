...
static int load_imatrix(const std::string & imatrix_file, std::vector<std::string> & imatrix_datasets, std::unordered_map<std::string, std::vector<float>> & imatrix_data) {
    ...
    for (const auto & sc : sums_counts_for) {
        ...
        // Detect per-weight Fisher data: ne1 > 1 means [ne0, ne1] = [n_per_row, nrows]
        // Standard imatrix has ne1 = 1 (or ne1 = n_experts)
        // Per-weight Fisher has ne1 = nrows (d_out dimension)
        float max_count = 0.0f;
        for (int64_t j = 0; j < ne1; ++j) {
            // For per-weight data, counts has shape [1,1] with value 1.0
            // For standard imatrix, counts has shape [1, n_experts]
            const int64_t count_idx = (ggml_nelements(counts) > 1) ? j : 0;
            const float count = ((const float *) counts->data)[count_idx];
            if (count > 0.0f) {
...
int main(int argc, char ** argv) {
    ...
    if ((params.ftype == LLAMA_FTYPE_MOSTLY_IQ2_XS || params.ftype == LLAMA_FTYPE_MOSTLY_IQ2_XXS ||
        ...
        fprintf(stderr, "\n==========================================================================================================\n");
        fprintf(stderr, "WARNING: IQ1_S/IQ1_M/IQ2_S/IQ2_XXS/IQ2_XS/Q2_K_S without imatrix — proceeding (ablation mode)\n");
        fprintf(stderr, "==========================================================================================================\n\n\n");
    }
