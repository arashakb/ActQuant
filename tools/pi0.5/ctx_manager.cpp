...
ContextManager::ContextManager(const std::string &dev_name, int n_threads, int max_nodes)
...
ggml_backend_t ContextManager::try_init_backend(enum ggml_backend_dev_type type) {
    return ggml_backend_init_by_type(type, nullptr);
}

bool ContextManager::create_backend(const std::string &dev_name, int n_threads) {
    backend_cpu_.reset(ggml_backend_init_by_type(GGML_BACKEND_DEVICE_TYPE_CPU, nullptr));
    if (!backend_cpu_.get()) {
    ...
    if (!dev_name.empty()) {
        ...
        if (dev) {
            ...
            fprintf(stderr, "%s: backend %s not found, available:\n", __func__, dev_name.c_str());
    ...
    if (!backend_) {
        fprintf(stderr, "%s: trying GPU backends\n", __func__);
    ...
    if (!backend_) {
        fprintf(stderr, "%s: no GPU found, using CPU\n", __func__);
    ...
    fprintf(stderr, "%s: using %s backend\n", __func__, ggml_backend_name(backend_.get()));
    ...
    if (backend_) {
        ...
        backend_buft_.push_back(ggml_backend_get_default_buffer_type(backend_.get()));
    }
    if (!ggml_backend_is_cpu(backend_.get())) {
        backend_ptrs_.push_back(backend_cpu_.get());
        backend_buft_.push_back(ggml_backend_get_default_buffer_type(backend_cpu_.get()));
    }
...
bool ContextManager::create_scheduler() {
    sched_.reset(ggml_backend_sched_new(backend_ptrs_.data(), backend_buft_.data(),
...
bool ContextManager::alloc_tensors() {
    if (!ctx_data_) {
        fprintf(stderr, "%s: ctx_data_ is null\n", __func__);
        return false;
    }
    buffer_.reset(ggml_backend_alloc_ctx_tensors(ctx_data_.get(), backend_.get()));
    if (!buffer_.get()) {
